...
class SetupController extends Controller
{
    ...
    public function configureSunat(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('certificate_file')) {
                ...
                // Usar nueva estructura: empresas/{RUC}/certificado/
                $certDirectory = StoragePathHelper::certificatePath($company->ruc);
                $path = $certificateFile->storeAs($certDirectory, 'certificado.pem', 'public');
    ...
    private function prepareCompanyData(Request $request): array
    {
        ...
        if ($request->hasFile('certificado_pem')) {
            ...
            // Usar nueva estructura: empresas/{RUC}/certificado/
            $certDirectory = StoragePathHelper::certificatePath($companyData['ruc']);

            Log::info('Guardando certificado en prepareCompanyData', [
                'ruc' => $companyData['ruc'],
                'directory' => $certDirectory,
                'full_path' => $certDirectory . '/certificado.pem'
            ]);
        ...
        if ($request->hasFile('logo_path')) {
            ...
            $logoDirectory = StoragePathHelper::logoPath($companyData['ruc']);

            Log::info('Guardando logo en prepareCompanyData', [
                'ruc' => $companyData['ruc'],
            ...
            $logoPath = $logoFile->storeAs($logoDirectory, $fileName, 'public');

...
class GreenterService
{
    ...
    protected function initializeSee(): See
    {
        ...
        // Configurar certificado cargando desde archivo de la empresa
        try {
            // Usar el certificado específico de la empresa desde la BD
            if (empty($this->company->certificado_pem)) {
                throw new CertificateException("La empresa {$this->company->razon_social} no tiene certificado configurado");
            }

            // La ruta del certificado está guardada en la BD (ej: empresas/20161515649/certificado/certificado.pem)
            ...
            Log::info("Intentando cargar certificado", [
                ...
                'certificado_pem_db' => $this->company->certificado_pem,
                'full_path' => $certificadoPath
            ]);
            ...
            Log::info("Certificado cargado y validado correctamente", [
                'company_id' => $this->company->id,
                'ruc' => $this->company->ruc,
    ...
    protected function initializeSeeApi()
    {
        ...
        // Configurar certificado para GRE
        try {
            // Usar el certificado específico de la empresa desde la BD
            if (empty($this->company->certificado_pem)) {
                throw new Exception("La empresa {$this->company->razon_social} no tiene certificado configurado para GRE");
            }

            // La ruta del certificado está guardada en la BD (ej: empresas/20161515649/certificado/certificado.pem)
            $certificadoPath = storage_path('app/public/' . $this->company->certificado_pem);

            Log::info("Intentando cargar certificado para GRE", [
                'company_id' => $this->company->id,
                'ruc' => $this->company->ruc,
                'certificado_pem_db' => $this->company->certificado_pem,
                'full_path' => $certificadoPath
            ...
            Log::info("Certificado GRE cargado correctamente", [
                'company_id' => $this->company->id,
                'ruc' => $this->company->ruc,
                'path' => $certificadoPath
            ]);

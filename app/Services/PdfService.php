...
class PdfService
{
    ...
    const FORMATS = [
        ...
        '80mm' => ['width' => 80, 'height' => 0, 'unit' => 'mm'], // Ticket ancho - altura automática
        '58mm' => ['width' => 58, 'height' => 0, 'unit' => 'mm'], // Ticket angosto - altura automática
    ];
    ...
    protected function prepareInvoiceData($invoice): array
    {
        ...
        return [
            ...
            'fecha_emision' => $invoice->fecha_emision ? $invoice->fecha_emision->format('d/m/Y h:i:s A') : date('d/m/Y h:i:s A'),
    ...
    protected function prepareBoletaData($boleta): array
    {
        ...
        return [
            ...
            'fecha_emision' => $boleta->fecha_emision->format('d/m/Y h:i:s A'),
    ...
    protected function prepareCreditNoteData($creditNote): array
    {
        ...
        return [
            ...
            'fecha_emision' => $creditNote->fecha_emision->format('d/m/Y h:i:s A'),
    ...
    protected function prepareDebitNoteData($debitNote): array
    {
        ...
        return [
            ...
            'fecha_emision' => $debitNote->fecha_emision->format('d/m/Y h:i:s A'),
    ...
    {
        // IMPORTANTE: Usar los valores ya calculados en la BD (que vienen de Greenter)
        // en lugar de recalcular desde los detalles, porque las operaciones gratuitas
        // NO deben sumarse al subtotal

        $subtotal = $invoice->mto_oper_gravadas ?? 0; // Solo operaciones gravadas (excluye gratuitas)
        $igv = $invoice->mto_igv ?? 0;
        $total = $invoice->mto_imp_venta ?? 0;
    ...
    protected function setPaperFormat(Dompdf $pdf, string $format): void
    {
        ...
        if ($formatLower === 'a4' || $formatLower === 'a5') {
            ...
            // Para formatos de ticket (80mm, 58mm)
            // Usar altura suficiente para evitar saltos de página
            $width = $this->mmToPt($formatConfig['width']);
            $height = $this->mmToPt(400); // Altura grande para papel continuo
    ...
    protected function prepareRetentionData($retention): array
    {
        ...
        return [
            ...
            'fecha_emision' => $retention->fecha_emision->format('d/m/Y h:i:s A'),
    ...
    protected function prepareNotaVentaData($notaVenta): array
    {
        ...
        return [
            ...
            'fecha_emision' => $notaVenta->fecha_emision->format('d/m/Y h:i:s A'),

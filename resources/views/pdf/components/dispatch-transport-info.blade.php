...
@if(in_array($format, ['a4', 'A4', 'a5', 'A5']))
    ...
    <div class="transport-info">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 9px;">
            <tr>
                <td style="padding: 4px 8px; width: 50%; border-right: 1px solid #ccc;">
                    <strong>MOTIVO TRASLADO:</strong> {{ $motivo_traslado }}
                </td>
                <td style="padding: 4px 8px; width: 50%;">
                    <strong>MODALIDAD:</strong> {{ $modalidad_traslado }}
                </td>
            </tr>
            <tr>
                <td style="padding: 4px 8px; border-right: 1px solid #ccc;">
                    <strong>PESO TOTAL:</strong> {{ $peso_total_formatted }}
                </td>
                <td style="padding: 4px 8px;">
                    @if(!empty($document->num_bultos))
                        <strong>N° BULTOS:</strong> {{ $document->num_bultos }}
                    @else
                        &nbsp;
                    @endif
                </td>
    ...
    <div class="addresses-info">
        <div class="address-section">
            ...
            @php
                $partidaDireccion = $document->partida['direccion'] ?? $document->partida_direccion ?? 'N/E';
                $partidaUbigeo = $document->partida['ubigeo'] ?? $document->partida_ubigeo ?? '';
            @endphp
            <p>{{ $partidaDireccion }}</p>
            @if($partidaUbigeo)
                <p style="font-size: 8px; color: #666;">Ubigeo: {{ $partidaUbigeo }}</p>
            @endif
        </div>
        <div class="address-section">
            <h4>PUNTO DE LLEGADA</h4>
            @php
                $llegadaDireccion = $document->llegada['direccion'] ?? $document->llegada_direccion ?? 'N/E';
                $llegadaUbigeo = $document->llegada['ubigeo'] ?? $document->llegada_ubigeo ?? '';
            @endphp
            <p>{{ $llegadaDireccion }}</p>
            @if($llegadaUbigeo)
                <p style="font-size: 8px; color: #666;">Ubigeo: {{ $llegadaUbigeo }}</p>
            @endif
        </div>
    </div>
